...
def evaluate(video_path,checkpoints,environment_config:dict,model_config:dict,device="cuda:0",base_url=None):
    ...
    config_base = environment_config.get("config_base", "./config")
    abs_config_path = os.path.join(config_base, str(config_path.parent))
    if os.path.isabs(abs_config_path):
        hydra.initialize_config_dir(config_dir=abs_config_path, version_base='1.3')
    else:
        hydra.initialize(config_path=abs_config_path, version_base='1.3')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config-base',type=str,default="./config")
